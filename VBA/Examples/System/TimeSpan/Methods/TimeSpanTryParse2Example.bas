...
Public Sub TimeSpanTryParse2()
    ...
    header = VBString.Format("{0,-17}", "String")
    ...
    For Each varCulture In cultures
        ...
        header = header + IIf(culture.Equals(CultureInfo.InvariantCulture), _
                            VBString.Format("{0,20}", "Invariant"), _
                            VBString.Format("{0,20}", culture.Name))
    Next
    Debug.Print header
    Debug.Print
    ...
    For Each value In values
        Debug.Print VBString.Format("{0,-17}", value);
        For Each varCulture In cultures
            Set culture = varCulture
            Dim interval As DotNetLib.TimeSpan
            If (TimeSpan.TryParse2(value, culture, interval)) Then
                Debug.Print VBString.Format("{0,20}", interval.ToString2("c"));
            Else
                Debug.Print VBString.Format("{0,20}", "Unable to Parse");
            End If
        Next
        Debug.Print
