...
Public Sub ArrayGetLowerBound()
    ...
    Debug.Print VBString.Format("Elements from index {0} to {1}:", lower, upper)
    ' Iterate the array.
    Dim ctr As Long
    ...
    Debug.Print VBString.Format("Number of dimensions: {0}", pvtRank)
    For ctr = 0 To pvtRank - 1
        Debug.Print VBString.Format("   Dimension {0}: ", ctr) & _
                    VBString.Format("from {0} to {1}", integers2d.GetLowerBound(ctr), integers2d.GetUpperBound(ctr))
    Next ctr
