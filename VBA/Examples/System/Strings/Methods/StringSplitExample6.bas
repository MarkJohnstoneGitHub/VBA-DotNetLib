...
Public Sub StringSplitExample6()
    ...
    Debug.Print VBString.Format(VBString.Unescape("Splitting the string:\n   ""{0}""."), source)
    Debug.Print
    Debug.Print VBString.Format(VBString.Unescape("Using the delimiter string:\n   ""{0}"""), stringSeparators(0))
    Debug.Print

    ' Split a string delimited by another string and return all elements.
    result = source.Split3(stringSeparators, StringSplitOptions.StringSplitOptions_None)
    Debug.Print VBString.Format("Result including all elements ({0} elements):", UBound(result) + 1)
    Debug.Print "   ";
    Dim s As Variant
    For Each s In result
        Debug.Print VBString.Format("'{0}' ", IIf(VBString.IsNullOrEmpty(CStr(s)), "<>", s));
    Next
    Debug.Print
    Debug.Print
    ...
    Debug.Print VBString.Format("Result including non-empty elements ({0} elements):", UBound(result) + 1)
    Debug.Print "   ";
    For Each s In result
        Debug.Print VBString.Format("'{0}' ", IIf(VBString.IsNullOrEmpty(CStr(s)), "<>", s));
    Next
