...
Public Sub SortedListCopyTo()
    ...
    For Each s In tempArray
        ...
        Call DictionaryEntry.Assign(dictEntry, i, s)
...
' Note: To remove old version.
' DictionaryEntry.GetType() now returns mscorlib.DictionaryEntry type
' Added member to DictionaryEntry.Assign(mscorlib.DictionaryEntry, key, value) to simplify creating a DictionaryEntry
Public Sub SortedListCopyToPreviousVersion()
    ' Creates and initializes the source SortedList.
    Dim mySourceList As DotNetLib.SortedList
    Set mySourceList = SortedList.Create()
    Call mySourceList.Add(2, "cats")
    ...
    ' Displays the values of the target Array.
    Debug.Print "The target Array contains the following (before and after copying):"
    Call PrintValuesOld(myTargetArray, " ")
...
'Note old version remove
Private Sub PrintValuesOld(ByVal myArr As DotNetLib.Array, ByVal mySeparator As String)
    Dim i As Long
    For i = 0 To myArr.Length - 1
        Dim de As mscorlib.DictionaryEntry
        de = myArr(i)
...
' Note To remove C# example translated to VBA example
